...
nonisolated enum FitnessLevel: String, Codable, CaseIterable, Identifiable, Sendable {
  ...
  init(from decoder: Decoder) throws {
    ...
    if normalized.contains("athlete") {
      self = .athlete
    } else if normalized.contains("advanced") {
      self = .advanced
    } else if normalized.contains("intermediate") {
      self = .intermediate
