...
struct WorkoutPlan: nonisolated Codable, Identifiable, Sendable {
  var id: UUID
  var name: String
...
enum MuscleGroup: String, Codable, CaseIterable, Identifiable, Sendable {
  // General focus tags (AI can return these)
  case cardio = "Cardio"
  case cardiovascularHealth = "Cardiovascular Health"
  case cardiovascularEndurance = "Cardiovascular Endurance"
  ...
  var icon: String {
    switch self {
    case .cardio, .cardiovascularHealth, .cardiovascularEndurance, .cardiovascularSystem:
      return "heart.fill"
    case .core:
  ...
  init(from decoder: Decoder) throws {
    ...
    if let exact = MuscleGroup(rawValue: raw) {
      self = exact
      return
    }

    // Common synonyms / variations from model output
    if normalized == "cardio" || normalized.contains("cardio") {
      self = .cardio
    } else if normalized.contains("cardiovascular") && normalized.contains("health") {
      self = .cardiovascularHealth
    } else if normalized.contains("cardiovascular") && normalized.contains("endurance") {
      self = .cardiovascularEndurance
    } else if normalized.contains("cardiovascular") && normalized.contains("system") {
      self = .cardiovascularSystem
    } else if normalized == "core" || normalized.contains("core") {
      self = .core
    } else if normalized.contains("endurance") {
      self = .endurance
    } else if normalized.contains("fat") && normalized.contains("burn") {
      self = .fatBurning
    } else if normalized.contains("mobility") {
      self = .mobility
    } else if normalized.contains("recovery") {
      self = .recovery
    } else if normalized.contains("strength") {
      self = .strength
    } else if normalized.contains("full body") {
      self = .fullBody
    } else {
      self = .fullBody
    }
...
enum WorkoutType: String, Codable, CaseIterable, Identifiable, Sendable {
  ...
  init(from decoder: Decoder) throws {
    ...
    if let exact = WorkoutType(rawValue: raw) {
      self = exact
      return
    }

    if normalized.contains("hiit") {
      self = .hiit
    } else if normalized.contains("strength") {
      self = .strength
    } else if normalized.contains("hypertrophy") || normalized.contains("muscle") {
      self = .hypertrophy
    } else if normalized.contains("endurance") {
      self = .endurance
    } else if normalized.contains("flex") || normalized.contains("mobility") {
      self = .flexibility
    } else if normalized.contains("recovery") || normalized.contains("liss") {
      self = .recovery
    } else if normalized.contains("circuit") {
      self = .circuit
    } else if normalized.contains("power") || normalized.contains("explosive") {
      self = .power
    } else if normalized.contains("cardio") {
      self = .cardio
    } else {
      self = .strength
    }
...
extension WorkoutPlan {
  ...
  static var sample: WorkoutPlan {
    WorkoutPlan(
      ...
      description:
        "A balanced 3-day full body program designed for beginners focusing on building strength foundation and proper movement patterns.",
      durationWeeks: 8,
      daysPerWeek: 3,
      workoutDays: [
        WorkoutDay(
          ...
          exercises: [
            Exercise(
              ...
              rpe: 7,
              instructions:
                "Stand with feet shoulder-width apart, bar on upper back. Descend by breaking at hips and knees, keeping chest up. Drive through heels to stand.",
            ...
            Exercise(
              ...
              rpe: 7,
              instructions:
                "Lie on bench, grip bar slightly wider than shoulders. Lower to chest with control, press up to lockout.",
            ...
            Exercise(
              ...
              rpe: 7,
              instructions:
                "Hinge at hips, back flat, pull bar to lower chest/upper abs. Lower with control.",
        ...
        WorkoutDay(
          ...
          exercises: [
            Exercise(
              ...
              restSeconds: 90,
              instructions:
                "Hold bar at hips, push hips back while keeping legs nearly straight. Feel stretch in hamstrings, return to start."
            ),
            Exercise(
              name: "Overhead Press",
              ...
              restSeconds: 90,
              instructions:
                "Press bar from shoulders to overhead, fully extending arms. Lower with control."
            ),
            Exercise(
              name: "Lat Pulldown",
              muscleGroups: [.back, .biceps],
              ...
              restSeconds: 60,
              instructions:
                "Pull bar to upper chest, squeezing shoulder blades together. Return with control."
            ),
          ],
          estimatedDuration: 45
        ),
      ...
      restDayGuidelines:
        "Take complete rest or do light activity like walking. Focus on sleep and nutrition for recovery.",
      warmupGuidelines:
        "5-10 minutes of light cardio followed by dynamic stretches. Do 1-2 warm-up sets of each exercise with light weight.",
      cooldownGuidelines:
        "5 minutes of light cardio, followed by static stretching of worked muscle groups.",
      progressionNotes:
        "Add 5lbs to lifts when you can complete all sets at the top of the rep range with good form. Deload every 4th week.",
