...
final class ClaudeAIService: AIServiceProtocol {
  ...
  private func parseResponse(_ data: Data) throws -> AIHealthRecommendationResponse {
    do {
      ...
      do {
        let recommendation = try AIJSONCoding.makeDecoder().decode(
        ...
        #if DEBUG
        print("[AI][Claude] decode failed: \(AIJSONCoding.debugDescribeDecodingError(error))")
        print("[AI][Claude] json chars: \(jsonString.count)")
        print("[AI][Claude] \(AIJSONCoding.debugSummarizeTopLevelJSON(from: jsonData))")
        print("[AI][Claude] json snippet:\n\(AIJSONCoding.debugSnippet(jsonString))")
        #endif
        throw error
      }
