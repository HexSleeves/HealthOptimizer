    ...
    private func supplementList(_ plan: SupplementPlan) -> some View {
        ScrollView {
            VStack(spacing: 20) {
                ...
                let filteredSupplements = selectedTiming == nil ? 
                    plan.supplements : 
                    plan.supplements.filter { $0.timing == selectedTiming }
                
                let sortedSupplements = filteredSupplements.sorted { $0.priority.sortOrder < $1.priority.sortOrder }
...
#Preview {
    SupplementsView(recommendation: HealthRecommendation.sample)
}
