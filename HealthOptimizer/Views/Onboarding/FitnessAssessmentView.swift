    ...
    ScrollView {
      VStack(spacing: 24) {
        ...
        SectionHeader(
          ...
          subtitle:
            "Help us understand your current fitness level and exercise preferences to create the right workout plan."
        )
            ...
            ForEach(0...7, id: \.self) { days in
              Button(action: { viewModel.weeklyActivityDays = days }) {
                Text("\(days)")
                  ...
                  .background(
                    viewModel.weeklyActivityDays == days ? Color.accentColor : Color(.systemGray5)
                  )
                  .foregroundColor(viewModel.weeklyActivityDays == days ? .white : .primary)
                  .cornerRadius(8)
              }
            }
          }
