      ...
      VStack(spacing: 24) {
        ...
        ReviewSection(title: "Basic Information", icon: "person.fill") {
          ...
          ReviewRow(
            label: "BMI",
            value: String(
              format: "%.1f (%@)", viewModel.calculatedBMI, viewModel.bmiCategory.rawValue)
          )
        }
        ...
        ReviewSection(title: "Fitness Profile", icon: "figure.run") {
          ...
          ReviewRow(
            label: "Workout Time", value: "\(viewModel.availableWorkoutMinutes) min/session")
          if !viewModel.selectedActivities.isEmpty {
            let activities = viewModel.selectedActivities.map { $0.rawValue }.joined(
              separator: ", ")
        ...
        ReviewSection(title: "Dietary Preferences", icon: "fork.knife") {
          ...
          if !viewModel.foodIntolerances.isEmpty {
            ReviewRow(
              label: "Intolerances", value: viewModel.foodIntolerances.joined(separator: ", "))
          }
        }
        ...
        ReviewSection(title: "Lifestyle", icon: "moon.stars.fill") {
          ReviewRow(
            label: "Sleep", value: String(format: "%.1f hrs/night", viewModel.averageSleepHours))
          ...
          ReviewRow(
            label: "Water Intake",
            value: String(format: "%.1fL/day", viewModel.dailyWaterIntakeLiters))
        }
        ...
        VStack(spacing: 12) {
          ...
          Text(
            "Your health data is stored securely on your device. When generating recommendations, only anonymized health metrics are sent to the AI service - never your personal information."
          )
          .font(.caption)
          .foregroundColor(.secondary)
          .multilineTextAlignment(.center)
